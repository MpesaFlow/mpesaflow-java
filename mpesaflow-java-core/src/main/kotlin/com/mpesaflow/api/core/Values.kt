...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        when (this) {
            is JsonString -> value
            else -> throw MpesaflowInvalidDataException("Value is not a string")
        }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw MpesaflowInvalidDataException("'${name}' is not set")
            is JsonNull -> throw MpesaflowInvalidDataException("'${name}' is null")
            else -> throw MpesaflowInvalidDataException("'${name}' is invalid, received ${this}")
        }
