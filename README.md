...
PaymentPaybillParams params = PaymentPaybillParams.builder()
    .accountReference("accountReference")
    .amount(0)
    .phoneNumber("phoneNumber")
    .transactionDesc("transactionDesc")
    .build();
PaymentPaybillResponse response = client.payments().paybill(params);
```
...
```java
PaymentPaybillResponse response = client.payments().paybill().validate();
```
...
```java
JsonValue secret = paymentPaybillResponse._additionalProperties().get("secret_field");
```
