...
[![Maven Central](https://img.shields.io/maven-central/v/com.mpesaflow.api/mpesaflow-java)](https://central.sonatype.com/artifact/com.mpesaflow.api/mpesaflow-java/0.0.1-alpha.1)
...
```kotlin
implementation("com.mpesaflow.api:mpesaflow-java:0.0.1-alpha.1")
```
...
Use `MpesaflowOkHttpClient.builder()` to configure the client. At a minimum you need to set `.apiKey()`:
...
Alternately, set the environment with `MPESAFLOW_API_KEY`, and use `MpesaflowOkHttpClient.fromEnv()` to read from the environment.
...
To create a new payment, first use the `PaymentCreateParams` builder to specify attributes,
then pass that to the `create` method of the `payments` service.
...
In [Example: creating a resource](#example-creating-a-resource) above, we used the `PaymentCreateParams.builder()` to pass to
the `create` method of the `payments` service.
...
```java
Payment payment = client.payments().create().validate();
```
...
```java
JsonValue secret = payment._additionalProperties().get("secret_field");
```
